...
agent_communication:
    ...
    -agent: "main"
    -message: "NEW: Real-time WebSocket job tracking added. WS endpoint /api/ws/track/{job_id}?token=JWT. Please backend-test (use User-Agent header 'Mozilla/5.0' to avoid WAF 403 on the python client): (a) connect as customer & driver of an ACTIVE accepted job -> both get {type:'ready',role}; (b) driver sends {type:'location',lat,lng,heading} -> customer receives {type:'location',...} and job.driver_location persists (GET /api/jobs/{id} shows driver_location); (c) connection with no/invalid token closes with code 4401; (d) a user who is NEITHER customer nor driver of the job is rejected 4403; (e) a customer (non-driver) sending a location message is IGNORED (not broadcast). Main agent already verified happy path E2E manually. Also confirm no regression on the prior job lifecycle/payments/connect endpoints."